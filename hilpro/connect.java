   ...
   public class connect
   {
    ...
    public final int tableColumns = columns.length;
       ...
       String[][] Show_all()throws Exception
        ...
        String[][] search_all(String search)throws Exception
        {
        int count = countRows();
        String data[][] = new String[count][tableColumns];
        String query = "SELECT * FROM student WHERE '"+search+"' in (id,name,fees,mobileNumber,course,courseDuration,fathersName,mothersName,age,dob,startDate,endDate);";
        try {
            rs = stm.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
